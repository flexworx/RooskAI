...
TEMPLATES: dict[str, dict[str, Any]] = {
    ...
    "opnsense-firewall": {
        ...
        "category": "networking",
        "description": "OPNsense-based software firewall with web UI. Provides stateful packet inspection, VLAN routing, port forwarding, IDS/IPS via Suricata, and VPN. Managed via a clean browser-based dashboard — no command line required.",
        "icon": "shield",
        "vm_config": {
            "cores": 2,
            "ram_mb": 2048,
        ...
        "requires_iso": True,
        "iso_name": "OPNsense-25.1-dvd-amd64.iso",
        "proxmox_overrides": {
            "ostype": "other",
            "cpu": "host",
        },
        "post_install_notes": [
            "Attach the OPNsense ISO via Proxmox console, then boot the VM",
            "Login with installer / opnsense, follow the install wizard",
            "After install: open https://<vm-ip> in your browser (admin / opnsense)",
            "Change the default password immediately in System → General",
            "Configure WAN/LAN interfaces under Interfaces → Assignments",
            "Enable IDS/IPS under Services → Intrusion Detection → Administration",
            "Set firewall rules under Firewall → Rules — block everything by default, allow only what you need",
            "For VPN: Services → WireGuard or OpenVPN wizard",
        ],
    },
    "dev-environment": {
