#!/bin/bash
# =============================================================================
# BYRTH BOOTSTRAP SCRIPT — Run from laptop AFTER Proxmox is installed
#
# This single script bootstraps the entire Byrth NexGen Platform:
...
#   6. Deploys Byrth (Docker) on App VM
#   7. Seeds the database and prints access URL
#
# Usage:
#   ssh root@192.168.4.58 "bash -s" < bootstrap-byrth.sh
#   — OR —
#   scp bootstrap-byrth.sh root@192.168.4.58:/root/ && \
#   ssh root@192.168.4.58 "bash /root/bootstrap-byrth.sh"
#
# Prerequisites:
...
echo "============================================="
echo "  BYRTH NexGen Platform — Bootstrap Script"
...
SSH_KEY_PATH="/root/.ssh/id_ed25519_byrth"
if [ ! -f "$SSH_KEY_PATH" ]; then
    ssh-keygen -t ed25519 -f "$SSH_KEY_PATH" -N "" -C "byrth-bootstrap"
...
cat > /tmp/byrth-db-userdata.yml <<DBCLOUD
#cloud-config
package_update: true
packages:
  - postgresql-16
...
cat > /tmp/byrth-app-userdata.yml <<APPCLOUD
#cloud-config
package_update: true
packages:
  - docker.io
...
# Create the VMs
create_cloud_vm "$DB_VMID" "$DB_NAME" "$DB_CORES" "$DB_RAM" "$DB_DISK" 20 "$DB_IP" "$DB_GW" "/tmp/byrth-db-userdata.yml"
create_cloud_vm "$APP_VMID" "$APP_NAME" "$APP_CORES" "$APP_RAM" "$APP_DISK" 20 "$APP_IP" "$APP_GW" "/tmp/byrth-app-userdata.yml"
...
# ─── Phase 6: Deploy Byrth on App VM ───────────────────────────
echo ""
echo "=== Phase 5: Deploy Byrth Platform ==="
...
scp -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$SSH_KEY_PATH" "deploy@${APP_IP}:/tmp/byrth_key"
ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "deploy@${APP_IP}" "sudo mv /tmp/byrth_key /opt/nexgen/.ssh/id_ed25519 && sudo chmod 600 /opt/nexgen/.ssh/id_ed25519"

# Copy the Byrth repo to the App VM
...
elif [ -d "/root/Byrth" ]; then
    REPO_DIR="/root/Byrth"
else
    echo "Cloning Byrth repo..."
    # If no local copy, try to clone from GitHub
    read -rp "GitHub repo URL (e.g. https://github.com/user/Byrth.git): " REPO_URL
    git clone "$REPO_URL" /opt/nexgen-repo
    REPO_DIR="/opt/nexgen-repo"
fi

echo "Copying Byrth repo to App VM..."
tar czf /tmp/byrth-repo.tar.gz -C "$REPO_DIR" .
scp -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no /tmp/byrth-repo.tar.gz "deploy@${APP_IP}:/tmp/"
ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "deploy@${APP_IP}" "
    sudo mkdir -p /opt/nexgen && \
    sudo tar xzf /tmp/byrth-repo.tar.gz -C /opt/nexgen && \
...
echo ""
echo "============================================="
echo "  BYRTH NexGen Platform — Bootstrap Complete"
echo "============================================="
echo ""
